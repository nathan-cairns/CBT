...
def plot_variable_stats(eval_objects):
    ...
    python_times_var_was_introduced = []

    one_to_use = c_times_var_was_introduced

    plt.bar(np.arange(len(one_to_use)), one_to_use)
    ...
    plt.title('Accuracy of model keyword and variable guessing across model training data splits')
    plt.xticks(np.arange(len(one_to_use)), ['Original files',
